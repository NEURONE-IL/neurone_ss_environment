...
import { BehaviorModelModalAddedComponent } from '../behaviormodel-modal-added/behaviormodel-modal-added.component';
...
@Component({
  selector: 'app-behaviormodel-new',
  templateUrl: './behaviormodel-new.component.html',
  styleUrls: ['./behaviormodel-new.component.css']
})

// New behavior model view
export class BehaviorModelNewComponent implements OnInit {
  ...
  // Constructor: Initializes the settings of this instance of the behavior model editor
  constructor(private fb: FormBuilder,
    ...
    this.modelWidth = EditorMainSettingsService.editorWidth;
    this.modelHeight = EditorMainSettingsService.editorHeight;
    this.scrollTop = 0;
    this.scrollLeft = 0;
  ...
  async ngOnInit() {
    ...
    // Adding the necessary start node to the diagram
  ...
  private initializePaperEvents = () => {
    ...
    this.paper.on('link:connect', (linkView: any) => {
      ...
      if (link.get('source').id && link.get('target').id) {
        ...
        // Prevents double links (in the same direction) between one node and another node
  ...
  public addBehaviorModel = () => {
    ...
    if (this.modelValid == true) {
      this._behaviorModelService.createBehaviorModel(BEHAVIORMODEL).subscribe(data => {
        ...
        console.log(error);
        this.router.navigate(['/', 'behaviormodel-new']);
      }
    } else {
  ...
  private openSuccessModal = () => {
    const dialogRef = this.dialog.open(BehaviorModelModalAddedComponent, { width: '25%', disableClose: true } );
    const sub = dialogRef.componentInstance.onSubmit.subscribe((value: any) => {
      dialogRef.close();
  ...
  // Navigates to the home page, discarding any changes made in the behavior model
  public discardModel = () => {
    this.router.navigate(['/', 'home'], { state: {
      goToBehaviorModelsTab: true
    }});
