...
// Interface which contains the data needed to add a query node to a behavior model
...
// Adds a query node to a behavior model (the graph and paper, provided as part of the input, will be modified directly, as they are passed by reference)
export function addQueryNode(jointInputParams: jointInputParameters): number {

  let newQueryText = $localize`:Default name of a new query node in the New Behavior Model and Behavior Model Edit components:New query`;
  ...
  let text = newQueryText + ' ' + queryCount.toString();
    ...
    for (let i = 0; i < diagram.cells.length; i++) {
      if (diagram.cells[i].type !== "link") {
        if (diagram.cells[i].attrs.label.text === text) {
          textValid = false;
          queryCount = queryCount + 1;
          text = 'New query '.concat(queryCount.toString());
        }
      }
    }
  }
  ...
  var queryNode = new joint.shapes.standard.BorderedImage({
    ...
    attrs: {
      ...
      image: {
        "xlink:href": "/assets/behaviormodel-query.png",
        refWidth: 0.7,
        refHeight: 0.7,
        refX: 0.15,
        refY: 0.15
