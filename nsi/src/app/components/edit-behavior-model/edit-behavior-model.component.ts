...
import { BehaviorModelEditedModalComponent } from '../behavior-model-edited-modal/behavior-model-edited-modal.component';
...
@Component({
  selector: 'app-edit-behavior-model',
  templateUrl: './edit-behavior-model.component.html',
  styleUrls: ['./edit-behavior-model.component.css']
})

export class EditBehaviorModelComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    // FIN SECCION RESTAURAR ELEMENT TOOLS A LOS NODOS YA EXISTENTES
  ...
  private openSuccessModal = () => {
    ...
    const dialogRef = this.dialog.open(BehaviorModelEditedModalComponent, { width: '25%' } );
    const sub = dialogRef.componentInstance.onSubmit.subscribe((value: any) => {
      dialogRef.close();
      this.router.navigate(['/', 'behavior-model-settings'], { state:
        { _id: this._id
